...
const appSecretInput = document.querySelector('#app-secret');
const serviceTypeInput = document.querySelector('#service-type-id');
const pageSizeInput = document.querySelector('#page-size');
const scoreThresholdInput = document.querySelector('#score-threshold');
...
const assertElement = (element, name) => {
  if (!element) {
...
const buildMatchResults = (items, songs, threshold) => {
  const results = [];
...
const renderResults = (matches, serviceTypeId) => {
...
const loadUnlinkedItems = async (plans, serviceTypeId) => {
  const unlinked = [];
